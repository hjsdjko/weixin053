	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`zixishileixing`, this.ruleForm.id);
...
//跨表计算判断
				var obj;
				if((!this.ruleForm.zixishileixing)){
					this.$utils.msg(`自习室类型不能为空`);
					return
				}
				//更新跨表属性
			       var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`zixishileixing`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`zixishileixing`, this.ruleForm);
						}else{
							await this.$api.add(`zixishileixing`, this.ruleForm);
						}
