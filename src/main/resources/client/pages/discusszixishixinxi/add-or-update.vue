	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusszixishixinxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusszixishixinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusszixishixinxi`, this.ruleForm);
						}else{
							await this.$api.add(`discusszixishixinxi`, this.ruleForm);
						}
						uni.setStorageSync('discusszixishixinxiCleanType',true);
